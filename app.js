...
app.get('/game/:user', function (req, res) {
	...
	request('http://api.steampowered.com/ISteamUser/ResolveVanityURL/v0001/?key=B1D8105F367F2C4FE1ACA9C1F64EC504&vanityurl=' + user,
...
var getGame = function(steamid, res) {
	request('http://api.steampowered.com/IPlayerService/GetOwnedGames/v0001/?key=B1D8105F367F2C4FE1ACA9C1F64EC504&steamid=' + steamid + '&format=json&include_appinfo=1',
		function (error, response, body) {
		  if (!error && response.statusCode == 200) {
		    var data = JSON.parse(body);
		    var games = [];
